...
parser.add_argument('-m', '--models', choices=['patchseq', 'm1_patchseq_total', 'm1_patchseq_exons', 'allen', 
                                               'celltype', 'm1_celltype_total', 'm1_celltype_exons', 'rf', 'rfhvg', 'mlp', 'mlphvg', 'eln', 'elnhvg'], 
                    default='m1_patchseq_exons', help='Input -m patchseq or -m celltype to designate models (default patchseq).')
...
# elif models == 'm1_patchseq_exons':
#     ref_embs_directory = 'm1_patchseq_exons_preds/'
#     model_tups = [['ElasticNet_emb_layer_preds/', 'prediction of AP width (ms)_log by embs alpha 0.01 l1 0.01 corr 0.529 p 5.313884416732613e-19 MAE 0.292.joblib'], 
#                   ['ElasticNet_emb_layer_preds/', 'prediction of ISI adaptation index_log by embs alpha 1.0 l1 0.01 corr 0.192 p 0.002607449854716161 MAE 0.746.joblib'], 
#                   ['ElasticNet_emb_layer_preds/', 'prediction of Upstroke-to-downstroke ratio_log by embs alpha 0.01 l1 0.01 corr 0.487 p 5.918500085107421e-16 MAE 0.817.joblib'], 
#                   ['ElasticNet_emb_layer_scores/', 'prediction of AP threshold (mV) by embs alpha 1.0 l1 0.3 corr 0.098 p 0.1272882757808288 MAE 5.294.joblib'], 
#                   ['ElasticNet_emb_layer_preds/', 'prediction of Membrane time constant (ms)_log by embs alpha 1.0 l1 0.01 corr 0.172 p 0.007128313136108976 MAE 5.409.joblib'], 
#                   ['ElasticNet_emb_layer_preds/', 'prediction of Sag ratio by embs alpha 0.01 l1 0.7 corr 0.12 p 0.06166602552473084 MAE 0.073.joblib'], 
#                   ['ElasticNet_emb_layer_preds/', 'prediction of Rheobase (pA)_log by pcs alpha 0.1 l1 0.1 corr 0.294 p 3.0657768775958574e-06 MAE 51.811.joblib'], 
#                   ['ElasticNet_emb_layer_preds/', 'prediction of AP amplitude (mV) by embs alpha 0.1 l1 0.01 corr 0.29 p 4.141947956583375e-06 MAE 8.49.joblib'], 
#                   ['ElasticNet_emb_layer_preds/', 'prediction of Latency (ms)_log by embs alpha 1.0 l1 0.01 corr -0.005 p 0.9372882860290995 MAE 53.491.joblib'], 
#                   ['ElasticNet_emb_layer_preds/', 'prediction of Input resistance (MOhm)_log by embs alpha 0.1 l1 0.1 corr 0.386 p 4.260788212175739e-10 MAE 84.914.joblib']]
# elif models == 'rf':
...
# Map the 'models' arg to the correct base directory
if models == 'allen':
    ref_embs_directory = 'allen_preds/'
elif models == 'patchseq':
    ref_embs_directory = 'combined_patchseq_all_preds/'
    ...
    ref_embs_directory = 'm1_patchseq_exons_preds/'
elif models in ['rf', 'rfhvg', 'mlp', 'mlphvg', 'eln', 'elnhvg']:
    ref_embs_directory = 'patchseq/'
elif models == 'm1_celltype_total':
    ref_embs_directory = 'm1_patchseq_total_preds/'
elif models == 'm1_celltype_exons':
...
# Dynamically glob the 'best__' files across all subdirectories
best_model_files = glob.glob(os.path.join(ref_embs_directory, '**', 'best__*.joblib'), recursive=True)

if not best_model_files:
    raise FileNotFoundError(f"No 'best__*.joblib' files found in {ref_embs_directory}. Please run select_best_models.py first.")

model_tups = []
for file_path in best_model_files:
    # Extract the subfolder name (e.g., 'ElasticNet_emb_layer_preds/') and the file name
    dir_name = os.path.basename(os.path.dirname(file_path)) + '/'
    file_name = os.path.basename(file_path)
    model_tups.append([dir_name, file_name])

for model_directory, model_name in tqdm(model_tups):
    # Strip the 'best__' prefix before parsing to preserve the original metadata
    clean_model_name = model_name.replace('best__', '')
    
    assert len(clean_model_name.split(' by ')) == 2
    y_name = clean_model_name.split(' by ')[0].replace('prediction of ', '')
    X_name = clean_model_name.split(' by ')[1].split(' ')[0]
    assert X_name in ['embs', 'pcs']
    print(y_name)
