...
# Load ephys features
df_ephys = pd.read_csv('m1_patchseq_ephys_features.csv', index_col='cell id')
...
df_ephys = df_ephys[ephys_list].dropna()
...
emb_layer = ['preds', 'scores'][1] # 'preds' or 'scores'
...
output_directory = f'{embs_directory}ElasticNet_emb_layer_{emb_layer}/'
...
df_ephys = df_ephys.loc[df_merged.index.tolist(), :]
...
for ephy in ephys_list:
    ...
    y_train = df_ephys.loc[cells_train, ephy].values
    y_test = df_ephys.loc[cells_test, ephy].values
